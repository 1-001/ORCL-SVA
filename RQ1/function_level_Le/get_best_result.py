...
cvss_col = ["AV", "AC", "PR", "UI", "S", "C", "I", "A", "cvss3_severity"]
...
for estimator in estimators:
    for leaf_node in leaf_nodes:
        ...
        val_mse_list = cur_df["val_mse"].tolist()
        val_mse = np.mean(val_mse_list)
        if val_mse < max_mse:
            max_mse = val_mse
            max_mse_para = parameters
df_lgbm = df[df["parameters"] == max_mse_para]
...
for estimator in estimators:
    for leaf_node in leaf_nodes:
        ...
        if val_mse < max_mse:
            max_mse = val_mse
            max_mse_para = parameters
df_rf = df[df["parameters"] == max_mse_para]
