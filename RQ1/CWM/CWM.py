...
def evaluate(clf, X_train_transformed, X_test_transformed, y_train, y_test):
    ...
    # Mean Zero Error
    mze = 1 - accuracy_score(y_test, y_pred)  # MZE = 1 - ACC

    return acc, precisionma, recallma, f1ma, mcc, mse, mze
...
def validate_data(clf, config, start_word_ngram, end_word_ngram):
    ...
    return (
        "{:.5f}\t{:.5f}\t{:.5f}\t{:.5f}\t{:.5f}\t{:.5f}\t{:.5f}\t{:.5f}".format(
            results[0], results[1], results[2], results[3], results[4], results[5], results[6], val_time
        )
    )
# Labels to evaluate
...
with open(result_file, 'w') as fout:
    for config in configs:
        ...
        for label in labels:
            ...
            print("Classifier\tAccuracy\tPrecision\tRecall\tF1\tMCC\tMSE\tMZE\tVal Time\n")
            fout.write("Classifier\tAccuracy\tPrecision\tRecall\tF1\tMCC\tMSE\tMZE\tVal Time\n\n")
