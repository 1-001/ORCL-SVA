...
df1 = pd.read_excel('C:\\Users\\Admin\\Desktop\\FVA-master\\Data\\function_level_Le_Data\\output\\data1.xlsx')
df2 = pd.read_excel('C:\\Users\\Admin\\Desktop\\FVA-master\\Data\\function_level_Le_Data\\output\\data2.xlsx')
df3 = pd.read_excel('C:\\Users\\Admin\\Desktop\\FVA-master\\Data\\function_level_Le_Data\\output\\data3.xlsx')
df4 = pd.read_excel('C:\\Users\\Admin\\Desktop\\FVA-master\\Data\\function_level_Le_Data\\output\\data4.xlsx')
df5 = pd.read_excel('C:\\Users\\Admin\\Desktop\\FVA-master\\Data\\function_level_Le_Data\\output\\data5.xlsx')

test1 = df1[df1['partition'] == 'test']
test2 = df2[df2['partition'] == 'test']
test3 = df3[df3['partition'] == 'test']
test4 = df4[df4['partition'] == 'test']
test5 = df5[df5['partition'] == 'test']

for i in range(1,6):
    # 读取xlsx文件
    print('-------------------------------------',i,'----------------------------------------')
    file_path = 'C:\\Users\\Admin\\Desktop\\FVA-master\\Data\\function_level_Le_Data\\output\\data'+str(i)+'.xlsx'
    df = pd.read_excel(file_path)

    # 确保你的数据集中有一列是用于划分partition的，例如 'partition' 列
    # 假设 'partition' 列的值为 'train'， 'valid' 和 'test'
    ...
    def validate_data(clf, config, start_word_ngram, end_word_ngram):
        ...
        X_train_transformed, X_valid_transformed, y_valid_filtered = feature_model(X_train, X_valid, y_valid, config,
        ...
        return "{:.5f}".format(results[0]) + "\t" + "{:.5f}".format(results[1]) + "\t" + "{:.5f}".format(
            results[2]) + "\t" + "{:.5f}".format(results[3]) + "\t" + "{:.5f}".format(
            results[4]) + "\t" + "{:.5f}".format(val_time)

        # Validate data (no cross-validation, use provided datasets)


    def test_data(clf, config, start_word_ngram, end_word_ngram):
        print("在第一个数据集上的性能")
        t_start = time.perf_counter()
        X_test1 = test1['processed_description'].values
        y_test1 = test1['Base Severity'].values
        X_train_transformed, X_test_transformed, y_test_filtered = feature_model(X_train, X_test1, y_test1, config,
                                                                                   start_word_ngram, end_word_ngram)
        results = evaluate(clf, X_train_transformed, X_test_transformed, y_train, y_test_filtered)
        val_time = time.perf_counter() - t_start
        return "{:.5f}".format(results[0]) + "\t" + "{:.5f}".format(results[1]) + "\t" + "{:.5f}".format(
            results[2]) + "\t" + "{:.5f}".format(results[3]) + "\t" + "{:.5f}".format(
            results[4]) + "\t" + "{:.5f}".format(val_time)
        ...
        for config in configs:
            ...
            for label in labels:
                ...
                for clf_name, clf in clfs.items():
                    print(clf_name + "\t", end='')
                    fout.write(clf_name + "\t")
                    val_res = test_data(clf, config, start_word_ngram, end_word_ngram)
                    print(val_res)
                    fout.write(val_res + "\n")
                fout.write("------------------------------------------------\n\n")
            fout.write("##############################################\n\n")
